...
public class LogActivity extends AppCompatActivity {
    ...
    private TextView    tvAzureStatus;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnAzure     = (Button)      findViewById(R.id.log_btn_azure);
        tvAzureStatus = (TextView)   findViewById(R.id.log_tv_azure_status);
        ...
        btnShare.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 2. Double automatique dans Azure Log Analytics
                tvAzureStatus.setText("Double Azure en cours…");
                tvAzureStatus.setTextColor(android.graphics.Color.parseColor("#FFA040"));
                btnAzure.setEnabled(false);
                AzureLogExporter.export(new AzureLogExporter.ExportCallback() {
                    @Override
                    public void onSuccess(final int count, final int httpStatus) {
                        runOnUiThread(new Runnable() {
                            @Override public void run() {
                                tvAzureStatus.setText("✅ Fichier partagé + "
                                        + count + " entrées Azure (HTTP " + httpStatus + ")");
                                tvAzureStatus.setTextColor(
                                        android.graphics.Color.parseColor("#44DD44"));
                                btnAzure.setEnabled(true);
                            }
                        });
                    }
                    @Override
                    public void onError(final String message) {
                        runOnUiThread(new Runnable() {
                            @Override public void run() {
                                tvAzureStatus.setText("⚠ Fichier partagé — Azure: ❌ " + message);
                                tvAzureStatus.setTextColor(
        ...
        btnAzure.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                btnAzure.setEnabled(false);
                tvAzureStatus.setText("Envoi en cours…");
                tvAzureStatus.setTextColor(android.graphics.Color.parseColor("#FFA040"));
                AzureLogExporter.export(new AzureLogExporter.ExportCallback() {
                    @Override
                    public void onSuccess(final int count, final int httpStatus) {
