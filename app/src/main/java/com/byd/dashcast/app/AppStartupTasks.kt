 ...
 * The `killOrphanSniffer` flag must be evaluated on the main thread before calling
 * [run] to preserve the "run once per process" invariant across Activity re-creates.
 */
        ...
        val t = Thread({
            ...
            if (killOrphanSniffer) {
                try {
                    ...
                    AppLogger.w(TAG, "orphan-sniffer cleanup failed: " + e.message)
                }
            }
        }, "storage-hygiene")
        t.isDaemon = true
