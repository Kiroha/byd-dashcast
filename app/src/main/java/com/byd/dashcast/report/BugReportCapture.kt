 ...
 *
 * Unlike the RE sniffer (which spawns long-lived background logcat processes), this is a one-shot
 * snapshot: it relies on the kernel logcat ring buffer, which already retains the last few minutes
 * of activity, so the report is retroactive (the moments before the bug are captured) without any
 * continuous logging.
 *
 * The shell dump (uid=2000) writes logcat + dumpsys snapshots to the app's external files dir; the
 * in-memory DashCast journal and a header are appended on the Java side. The logcat sections are
 * bounded by `-t`, and are written BEFORE the `tail` line ceiling — so the file stays a few MB even
 * on a log-spamming unit.
 *
 * SECTION ORDER IS LOAD-BEARING, and so is the size budget that protects it. The dump is read back
 * with a hard byte cap taken from the START of the file ([readFile]), so whatever is written LAST
 * is what a size overflow deletes. Logcat stays first and dumpsys second on purpose: the logcat
 * sections are ordered oldest→newest, so truncating them would eat the most RECENT lines — the very
 * moments being reported. The `-t` bounds keep the file well under the cap, and [readFile] shouts
 * if it is ever reached. Do not "fix" this by swapping the order; fix it by lowering the `-t`
 * bounds.
 *
 * Kotlin port note: every `$` in the shell dump below is escaped. An unescaped one is string
 * interpolation and would compile a DIFFERENT command with no error anywhere. The whole dump is
 * diffed byte-for-byte against the Java output.
 */
object BugReportCapture {

    private const val TAG = "BugReportCapture"

    /** Filename prefix for generated bug reports. Public because `AppLogger.PRUNED_PREFIXES`
    ...
    const val PREFIX = "byd_bugreport_"

    /** Line-count cap kept ONLY as the fallback for a ROM whose logcat refuses `-t <time>`. */
    private const val LOGCAT_FALLBACK_LINES = 5000

    /**
     * Hard byte cap applied when the shell dump is read back, taken from the START of the file.
     * The `-t` bounds on the logcat sections keep a real report an order of magnitude below this,
     * so it is a last-resort guard rather than a working limit — but it cuts at the END of the
     * file, so if it ever bites it removes the dumpsys sections. [readFile] therefore says so out
     * loud instead of letting a truncated report look like a device that produced nothing (which
     * is exactly how INC-20260727-203241 nearly went undiagnosed).
     */
    private const val REPORT_BODY_MAX_BYTES = 4L * 1024L * 1024L
     ...
     */
    private const val TRUNC_MARKER = "@@DASHCAST_BODY_TRUNCATED@@"

    /** Well beyond the 120 s capture timeout, so an active concurrent report is never swept. */
    private const val STAGED_REPORT_MAX_AGE_MIN = 15

    interface Callback {
        /** Called on the main thread with the finished file (never null on success). */
        fun onReady(file: File)
        /** Called on the main thread; `partial` is the best-effort file or null. */
        fun onError(message: String, partial: File?)
    }

    internal fun buildStagedCleanupCommand(): String =
            "find /data/local/tmp -name '" + PREFIX + "*.txt' -mmin +" +
                    STAGED_REPORT_MAX_AGE_MIN + " -delete 2>/dev/null"

    internal fun buildStagedRemovalCommand(path: String?): String {
        val root = "/data/local/tmp/$PREFIX"
        if (path == null || !path.startsWith(root)
                || !path.substring(root.length).matches(Regex("[A-Za-z0-9._-]+"))) {
            throw IllegalArgumentException("not a DashCast staged report path")
        }
    ...
    @JvmStatic
    fun pruneStagedReports(context: Context?) {
        if (context == null || Build.VERSION.SDK_INT < 33) return
        ShellGateway.execShell(context.applicationContext, buildStagedCleanupCommand())
    }

    private fun removeStagedReport(context: Context, path: String) {
        try {
            ShellGateway.execShell(context.applicationContext, buildStagedRemovalCommand(path))
        } catch (error: Throwable) {
            AppLogger.w(TAG, "staged report cleanup failed: " + error.message)
        }
    }
    ...
    @JvmStatic
    fun versionLine(): String = BuildConfig.VERSION_NAME + " (" + BuildConfig.VERSION_CODE + ")"

    /**
     * The shell dump, as one string, so what it asks for can be asserted.
     *
     * Extracted for one reason: every defect this command has had was a silently empty section,
     * never a crash. A `head` exhausted on the wrong display, a filterspec naming tags this ROM
     * does not carry, a grep alternation missing the service that actually logs — each produced a
     * report that looked complete and answered nothing, and none of them could fail a test while
     * the command was a local variable.
     *
     * @param p the path every section appends to.
     */
    internal fun buildShellDump(p: String): String {
        return "echo '=== DASHCAST BUG REPORT ===' > " + p +
             " ; date >> " + p +
    ...
    /**
     * Captures a bug-report file. `metaHeader` (the user's Title/Steps/Result block) is written at
     * the very top so it is readable without scrolling. Runs the shell dump in the background;
     * `cb` fires on the main thread.
     */
    @JvmStatic
    fun capture(context: Context, metaHeader: String?, cb: Callback) {
        val app = context.applicationContext
        val outFile = newFile(app)
        ...
        val stageInTmp = Build.VERSION.SDK_INT >= 33
        val p = if (stageInTmp) "/data/local/tmp/" + outFile.name else outFile.absolutePath
        ...
        if (AdbLocalClient.isAdbTransportUnreachable()) {
            val diagnosis = AdbLocalClient.adbTransportDiagnosis()
            val offline = Thread(
                    { finish(app, outFile, metaHeader, cb, diagnosis, null) },
                    "bugreport-offline")
            offline.isDaemon = true
            offline.start()
            return
        }
        ...
        if (stageInTmp) {
            ...
            // the staged path, so stdout is otherwise empty and `out` carries exactly the body.
            // Uses the non-logging shell path so the ~1 MB body never lands in the journal.
            //
        ...
        val dumpCb = object : AdbLocalClient.Callback {
            override fun onSuccess(out: String?) {
                ...
                finish(app, outFile, metaHeader, cb, null,
                        if (stageInTmp) applyStagedTruncation(out) else null)
            }
     ...
     *     path); when non-null it is used verbatim and `outFile` is NOT read (on A13 the app
     *     cannot read that path). When null, the body is read from `outFile` (A10).
     */
    // internal, not private, so RedactionCallSiteTest can drive it. That test is the only thing
    // that proves the artefact LEAVING this emitter is filtered: Redactor's own tests cover the
    // rules as a pure function, and would all still pass if the redact() call below were deleted.
    internal fun finish(app: Context, outFile: File, metaHeader: String?,
                        cb: Callback, shellError: String?, preReadShellBody: String?) {
        ...
        var content: String
        try {
            val shellBody = preReadShellBody
                    ?: (if (outFile.exists()) readFile(outFile) else "")
            val sb = StringBuilder(shellBody.length + 8192)
            sb.append(metaHeader ?: "").append("\n\n")
        ...
        try {
            val r = Redactor.redact(content)
            content = r.text
            redactionNote = r.summary()
        } catch (t: Throwable) {
            redactionNote = "REDACTION FAILED (" + t.javaClass.simpleName +
     ...
     */
    private fun hudStateSnapshot(): String {
        ...
        try {
            try { ProxyClient.canListenStart() } catch (ignore: Throwable) { /* ensure registered */ }
            val s = ProxyClient.canListenDrain()
     ...
     * [newFile] falls back here when getExternalFilesDir() throws, and reports land here on the
     * DL5.1 / Android 13 ROMs where it does — while AppLogger.pruneOldFiles resolved the same
     ...
     *
     * Same contract shape as [PREFIX] and AppLogger.PRUNED_PREFIXES: one definition, two readers.
     * Duplicating the literal is what let them drift the first time.
     */
    @JvmStatic
    fun canonicalExternalFilesDir(app: Context): File =
            File("/storage/emulated/0/Android/data/" + app.packageName + "/files")

    internal fun newFileName(): String {
        val ts = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ROOT).format(Date())
        return PREFIX + ts + "_" + UUID.randomUUID().toString().replace("-", "") + ".txt"
    }

    private fun newFile(app: Context): File {
        var dir: File? = null
        ...
        if (!dir.exists()) {
            try { dir.mkdirs() } catch (ignore: Throwable) { /* best-effort */ }
        }
        return File(dir, newFileName())
    }

    /**
     * Reads the shell dump back, capped at [REPORT_BODY_MAX_BYTES] bytes FROM THE START.
     *
     * The cap is a last-resort guard — the `-t` bounds keep the report well under it, so it should
     * never be reached. If it ever is, the cut lands at the END of the file, i.e. it silently
     * removes the dumpsys sections (DISPLAYS, WINDOW STACKS, SURFACEFLINGER, CAR SERVICE, daemon
     * logs), which is indistinguishable from "the device produced nothing" when triaging. So a
     * truncation now says so, in the report and in the journal.
     */
    @Throws(IOException::class)
    private fun readFile(f: File): String {
        val len = f.length()
        val buf = ByteArray(minOf(len, REPORT_BODY_MAX_BYTES).toInt())
     ...
     * ([readFile]) and the A13 staged read-back ([applyStagedTruncation]).
     *
     * @param actualBytes the dump's true size, or a negative value if it could not be read back.
     */
    private fun truncationBanner(actualBytes: Long): String =
            "\n\n!!!!!!!! REPORT TRUNCATED !!!!!!!!\n" +
                    "The shell dump was " +
                    (if (actualBytes >= 0) actualBytes.toString() + " bytes"
                     else "larger than the cap") +
     ...
     *
     * internal for the test: this is the only place that decides whether a DL5.1 report announces
     * its own truncation, and a report that stays silent about it is how a cut dump gets triaged
     * as "the device produced nothing".
     */
    internal fun applyStagedTruncation(out: String?): String? {
        if (out == null) return null
        val i = out.lastIndexOf(TRUNC_MARKER)
        ...
        var actual = -1L
        try {
            actual = out.substring(i + TRUNC_MARKER.length).trim().toLong()
        } catch (ignore: Throwable) { /* -1: the banner still says the body was cut */ }
    ...
    private fun post(app: Context, r: Runnable) {
