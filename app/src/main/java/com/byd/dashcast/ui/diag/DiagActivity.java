...
public class DiagActivity extends AppCompatActivity {
    ...
    private void wireDiagNavRail() {
        ...
        // v1.4.9-beta — Layouts
        NavRailLayouts.apply(this, R.id.nav_layouts_diag, true);
    }
    ...
    private final List<com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult> mirrorLastResults = new ArrayList<>();
    ...
    private void prepareMirrorTestRows() {
        ...
        LayoutInflater inflater = LayoutInflater.from(this);
        for (com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestDef def
                : com.byd.dashcast.cluster.mirror.MirrorTestRunner.catalog()) {
            View row = inflater.inflate(R.layout.item_beta_test, llMirrorTestList, false);
            com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult r =
                    new com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult(def);
            r.status = com.byd.dashcast.cluster.mirror.MirrorTestRunner.Status.PENDING;
            bindMirrorRow(row, r);
    ...
    private void bindMirrorRow(View row, com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult r) {
        TextView status = row.findViewById(R.id.tv_test_status);
        TextView id     = row.findViewById(R.id.tv_test_id);
        TextView title  = row.findViewById(R.id.tv_test_title);
        TextView desc   = row.findViewById(R.id.tv_test_description);
    ...
    private void runMirrorAllTests() {
        ...
        btnMirrorSendLog.setEnabled(false);
        com.byd.dashcast.cluster.mirror.MirrorTestRunner.runAll(this,
                new com.byd.dashcast.cluster.mirror.MirrorTestRunner.Listener() {
            @Override public void onSuiteStarted(
                    List<com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult> results) {
                if (mDestroyed) return;
                mirrorLastResults.clear();
            ...
            @Override public void onTestUpdated(int index,
                    com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult result) {
                if (mDestroyed) return;
    ...
    private void updateMirrorCounters() {
        int pass = 0, fail = 0, skip = 0, warn = 0;
        for (com.byd.dashcast.cluster.mirror.MirrorTestRunner.TestResult r : mirrorLastResults) {
            switch (r.status) {
                case PASS:    pass++; break;
                case FAIL:    fail++; break;
                case SKIPPED: skip++; break;
    ...
    private void sendMirrorLog() {
        String report = com.byd.dashcast.cluster.mirror.MirrorTestRunner.buildReport(this, mirrorLastResults);
