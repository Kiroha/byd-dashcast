...
class AdbKeyBackupRulesTest {
    ...
    @Test
    fun `both Android backup formats are strict preference allowlists`() {
        ...
        for (name in listOf("backup_rules.xml", "data_extraction_rules.xml")) {
            val document = DocumentBuilderFactory.newInstance().newDocumentBuilder()
                .parse(File(root, "app/src/main/res/xml/$name"))
            assertTrue(document.getElementsByTagName("exclude").length == 0)
        }
