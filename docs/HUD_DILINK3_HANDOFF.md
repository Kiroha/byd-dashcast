...
- **CAN→HUD arrow injection works on arrow-capable firmware — VIDEO-CONFIRMED (§5)** — the big one, now **locked**: a tester video shows the W-HUD rendering our bench sweep ↑STRAIGHT→↰LEFT→↱RIGHT with countdown distances (2 YES / 1 NO on SX326; NO procedure-dependent). Direction glyphs render correctly.
